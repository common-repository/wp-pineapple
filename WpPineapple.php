...
class WpPineapple {
	...
	public function WpPineappleHandleForm($comment) {
		...
		if(!isset($_POST['wp_pineapple_fruit']) || empty($_POST['wp_pineapple_fruit'])) {
			wp_die(__("You must click one of the fruits to prove you are human!"));
		}
		...
		if($selectedFruitHash != $correct_value) {
			...
			wp_die(__("That's not the correct fruit! You're a robot!"));
			exit;
		} else {
			return $comment;
		}
	...
	public function WpPineappleRenderFruit($post_id) {
		...
		$form_label = sprintf(__('Are you human? Click the %s...') , ucfirst($this->fruits[$random_fruit]));
